...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare("SELECT COUNT(*) FROM kontak WHERE telepon = ? OR (email IS NOT NULL AND email = ?)");
        $stmt->execute([$telepon, $email]);
        if ($stmt->fetchColumn() > 0) {
            $errors['telepon_duplicate'] = "Nomor Telepon atau Email sudah terdaftar.";
        }
    }

    if (empty($errors)) {
        try {
            $stmt = $pdo->prepare("INSERT INTO kontak (nama, telepon, email) VALUES (?, ?, ?)");
            $stmt->execute([$nama, $telepon, $email ?: null]); 
            
            $_SESSION['message'] = "Kontak $nama berhasil ditambahkan!";
...
<body class="p-8">
    <div class="max-w-lg mx-auto bg-white p-6 rounded-lg shadow-xl">
        <h2 class="text-2xl font-bold mb-6 text-gray-800 border-b pb-2">Tambah Kontak Baru</h2>
        ...
        <form method="POST">
            ...
            <div class="mb-4">
                ...
                <input class="shadow appearance-none border <?= isset($errors['telepon']) || isset($errors['telepon_duplicate']) ? 'border-red-500' : '' ?> rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="telepon" type="tel" name="telepon" value="<?= htmlspecialchars($telepon) ?>" required>
                <?php if (isset($errors['telepon'])): ?>
                    <p class="text-red-500 text-xs italic mt-1"><?= $errors['telepon'] ?></p>
                <?php elseif (isset($errors['telepon_duplicate'])): ?>
                    <p class="text-red-500 text-xs italic mt-1"><?= $errors['telepon_duplicate'] ?></p>
                <?php endif; ?>
            </div>
            ...
            <div class="flex items-center justify-between">
                <button class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline transition duration-150" type="submit">
                    Simpan Kontak
                </button>
